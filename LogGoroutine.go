...
type LogGoroutineCallback func(logger ILogger)

// LogGoroutine 执行callback,并在callback中发生panic时自动恢复并使用logger记录出错的信息
func LogGoroutine(logger ILogger,callback LogGoroutineCallback) {
	defer func(){
...
func AsyncLogGoroutine(logger ILogger,callback LogGoroutineCallback) {
